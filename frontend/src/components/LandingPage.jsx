...
const costColor = (t) => {
  if (!t) return { bg: "#F3F4F6", text: "#6B7280" };
  const l = t.toLowerCase().replace(/[_\s]/g, "");
  if (l === "low") return { bg: "#D1FAE5", text: "#065F46" };
  if (l === "moderate") return { bg: "#DBEAFE", text: "#1E40AF" };
  if (l === "high") return { bg: "#FEF3C7", text: "#92400E" };
...
const isStudentFree = (r) => {
  ...
  // Both endpoints must be in Germany (DE)
...
const LandingPage = () => {
  ...
  const readUserData = () => {
    ...
    return {
      ...
      cities: email
        ? (() => {
            try {
              return JSON.parse(localStorage.getItem("myCities") || "[]");
            } catch {
              return [];
            }
          })()
  ...
  const userCityData = CITY_COORDS.find(
    (c) => c.slug === userCity || c.name.toLowerCase() === userCity.toLowerCase()
  );
  ...
  useEffect(() => {
    if (!userEmail) return;
    axios
      .get(`${BACKEND}/user/profile?email=${encodeURIComponent(userEmail)}`, authHeaders())
      .then((res) => {
        const d = res.data;
        ...
        if (allPhrases.length) {
          const funPhrases = allPhrases.filter((p) => {
            ...
            const hasSlangTag =
              ...
              tags.includes("colloquial") ||
              tags.includes("local_feel") ||
              tags.includes("fun");
            const isInformalCat =
              cat === "exclamations" ||
              cat === "social" ||
              ...
              cat === "food_drink";
            const isInformalReg =
              reg === "informal" || reg === "casual" || reg === "colloquial";
  ...
  useEffect(() => {
    ...
    map.on("zoomend", () => {
      const z = map.getZoom();
      if (z >= 13 && currentBase === "street") {
        map.removeLayer(osmLayer);
        satLayer.addTo(map);
        currentBase = "satellite";
      } else if (z < 13 && currentBase === "satellite") {
    ...
    CITY_COORDS.forEach((pin) => {
      ...
      const starBadge = isUserCity
        ? `<div style="position:absolute;top:-10px;right:-10px;background:#FA4616;color:#fff;font-size:8px;font-weight:800;padding:2px 5px;border-radius:9999px;white-space:nowrap;box-shadow:0 2px 6px rgba(250,70,22,0.4);z-index:3;border:1.5px solid #fff">YOUR CITY</div>`
        : isSavedCity
        ? `<div style="position:absolute;top:-8px;right:-10px;background:${color};color:#fff;font-size:7px;font-weight:700;padding:2px 5px;border-radius:9999px;white-space:nowrap;box-shadow:0 2px 6px ${color}55;z-index:3;border:1.5px solid #fff">SAVED</div>`
        : "";

      const pinScale = isUserCity ? "scale(1.2)" : "scale(1)";

      const htmlContent = `
        <div class="mtb-pin" style="display:flex;flex-direction:column;align-items:center;cursor:pointer;position:relative;transform:${pinScale}">
          ${starBadge}
          <div class="mtb-pulse" style="position:absolute;top:4px;left:50%;transform:translateX(-50%);width:24px;height:24px;border-radius:50%;background:${color};opacity:0.25"></div>
          <svg width="36" height="46" viewBox="0 0 32 42" style="filter:drop-shadow(0 3px 6px rgba(0,0,0,0.35));position:relative;z-index:2;transition:transform 0.2s" onmouseover="this.style.transform='scale(1.15)'" onmouseout="this.style.transform='scale(1)'">
            ...
            <text x="16" y="19" text-anchor="middle" font-size="12">${flag}</text>
          </svg>
        </div>`;
      ...
      const tooltipContent = `
        <div style="font-family:Inter,system-ui,sans-serif;padding:12px 14px;min-width:240px;max-width:320px">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px">
            <strong style="font-size:0.95rem;color:#111827">${flag} ${pin.name}</strong>
            <span style="font-size:0.6rem;font-weight:700;color:#fff;background:${color};padding:2px 8px;border-radius:9999px">${COUNTRY_LABELS[pin.country]}</span>
          </div>
          <div style="font-size:0.78rem;color:#4B5563;line-height:1.5;margin-bottom:8px">${pin.desc}</div>
          ${pin.uni ? `<div style="font-size:0.72rem;color:#0021A5;font-weight:600;margin-bottom:8px">🏫 ${pin.uni}</div>` : ""}
          <div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:8px">
            ${pin.cost ? `<span style="font-size:0.62rem;font-weight:600;background:${costBg};color:${costCol};padding:3px 8px;border-radius:4px"><span style="opacity:0.7;font-size:0.55rem;display:block;line-height:1;margin-bottom:1px">AVG RENT</span>💰 ${pin.cost}</span>` : ""}
            ${pin.beer ? `<span style="font-size:0.62rem;font-weight:600;background:#FFF7ED;color:#92400E;padding:3px 8px;border-radius:4px"><span style="opacity:0.7;font-size:0.55rem;display:block;line-height:1;margin-bottom:1px">BEER</span>🍻 ${pin.beer}</span>` : ""}
            ${pin.transit ? `<span style="font-size:0.62rem;font-weight:600;background:#EEF2FF;color:#4338CA;padding:3px 8px;border-radius:4px"><span style="opacity:0.7;font-size:0.55rem;display:block;line-height:1;margin-bottom:1px">TRANSIT</span>🚆 ${pin.transit}</span>` : ""}
          </div>
          ${pin.mustDo ? `<div style="font-size:0.7rem;color:#059669;font-weight:600;background:#ECFDF5;padding:5px 8px;border-radius:6px;border:1px solid #A7F3D0;margin-bottom:6px">⭐ ${pin.mustDo}</div>` : ""}
          <div style="font-size:0.64rem;font-weight:600;color:#0021A5;text-align:center;padding-top:4px;border-top:1px solid #E5E7EB">Click for city details</div>
        </div>`;
    ...
    LANDMARKS.forEach((lm) => {
      const lIcon = L.divIcon({
        className: "",
        html: `
          <div style="display:flex;align-items:center;justify-content:center;width:30px;height:30px;border-radius:50%;background:#fff;border:2px solid #E5E7EB;box-shadow:0 2px 8px rgba(0,0,0,0.18);font-size:15px;cursor:pointer;transition:all 0.2s" onmouseover="this.style.transform='scale(1.25)';this.style.borderColor='#FA4616';this.style.boxShadow='0 4px 16px rgba(250,70,22,0.3)'" onmouseout="this.style.transform='scale(1)';this.style.borderColor='#E5E7EB';this.style.boxShadow='0 2px 8px rgba(0,0,0,0.18)'">
            ${lm.icon}
          </div>`,
        iconSize: [30, 30],
        iconAnchor: [15, 15],
      ...
      const tooltipContent = `
        <div style="font-family:Inter,system-ui,sans-serif;padding:10px 12px;max-width:300px">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:5px">
            <strong style="font-size:0.88rem;color:#111827">${lm.icon} ${lm.name}</strong>
            <span style="font-size:0.58rem;font-weight:700;color:#fff;background:${catColor};padding:2px 7px;border-radius:9999px;text-transform:uppercase;letter-spacing:0.05em">${catLabel}</span>
          </div>
          <div style="font-size:0.72rem;color:#6B7280;margin-bottom:6px">${lm.city}</div>
          ${lm.tip ? `<div style="font-size:0.74rem;color:#065F46;font-weight:500;background:#ECFDF5;padding:5px 8px;border-radius:6px;border:1px solid #A7F3D0;line-height:1.45;margin-bottom:6px">💡 ${lm.tip}</div>` : ""}
          <div style="font-size:0.68rem;text-align:center;padding-top:5px;border-top:1px solid #E5E7EB;color:#0021A5;font-weight:600">📍 Click to open in Google Maps</div>
        </div>`;

      lMarker.bindTooltip(tooltipContent, {
        direction: "top",
        offset: [0, -8],
        className: "city-tooltip",
      });
    ...
    TRAVEL_ROUTES.forEach((r) => {
      ...
      const line = L.polyline(
        [[fromCity.lat, fromCity.lng], [toCity.lat, toCity.lng]],
        {
          color: isFree ? "#10B981" : "#FA4616",
          weight: isFree ? 2.5 : 2,
          opacity: isFree ? 0.7 : 0.5,
          dashArray: isFree ? "4, 6" : "8, 8",
          className: "mtb-route",
        }
      );
      ...
      const tooltipContent = `
        <div style="font-family:Inter,system-ui,sans-serif;padding:8px 10px;text-align:center">
          ...
          <div style="display:flex;gap:8px;justify-content:center;margin-top:4px;font-size:0.72rem;color:#4B5563">
            <span>⏱ ${r.time}</span>
            <span style="${isFree ? "color:#059669;font-weight:600" : ""}">${isFree ? "🎓 Free" : r.price}</span>
          </div>
          ${r.type ? `<div style="font-size:0.65rem;color:#6B7280;margin-top:3px">${r.type}</div>` : ""}
      ...
      line.bindTooltip(tooltipContent, { sticky: true, className: "city-tooltip" });
  ...
  useEffect(() => {
    if (userEmail) {
      axios
        .get("/api/user/profile", { params: { email: userEmail } })
        .then((res) => {
  ...
  const saveAsFlashcard = async () => {
    ...
    try {
      await axios.post(
        `${BACKEND}/phrases/bookmarks`,
        { phrase_id: phraseOfDay._id, user_email: userEmail },
        authHeaders()
      );
      setFlashcardSaved(true);
    } catch (err) {
      if (err?.response?.status === 400) setFlashcardSaved(true);
  ...
  const findRoute = () => {
    ...
    let route = TRAVEL_ROUTES.find((r) => r.from === routeFrom && r.to === routeTo);
    if (!route) {
      route = TRAVEL_ROUTES.find((r) => r.from === routeTo && r.to === routeFrom);
    }
  ...
  const getPhraseRegionInfo = () => {
    ...
    return phraseOfDay.city_slugs.map((s) => {
      const c = CITY_COORDS.find((cc) => cc.slug === s);
      return c
        ? { name: c.name, country: COUNTRY_LABELS[c.country], flag: COUNTRY_FLAGS[c.country], slug: s }
        : { name: s, country: "", flag: "\uD83C\uDF0D", slug: s };
    });
  };
  ...
  // Render Helpers
  const grouped = cities.reduce((a, c) => {
    ...
    <div style={S.page}>
      ...
      <header style={S.hdr}>
        <div style={S.hdrIn}>
          ...
          <nav style={S.nav}>
            {[
              { path: "/tips", icon: <Compass size={15} />, label: "Explore" },
              { path: "/reservations", icon: <ClipboardList size={15} />, label: "Study" },
              { path: "/events", icon: <Calendar size={15} />, label: "Events" },
              { path: "/", icon: <MapPin size={15} />, label: "Home" },
            ].map(({ path, icon, label }) => {
            ...
            {userEmail ? (
              <>
                <button onClick={() => navigate("/profile")} style={S.nbA}>
              ...
              <button onClick={() => navigate("/login")} style={S.nbA}>
                Sign In
              </button>
            )}
          </nav>
      ...
      <section style={S.hero}>
        ...
        >
          {userName ? (
            ...
            <>
              <h1 style={S.heroT}>
                Your study abroad semester,<br />
                <span style={S.heroHL}>figured out</span>
              </h1>
              <p style={S.heroSub}>
                From your first awkward café order to your last Biergarten sunset — {TOTAL_CITIES} cities, three countries, zero guesswork.
              </p>
          ...
          <div style={S.heroActs}>
            <button
              ...
              onMouseEnter={(e) => {
                ...
                e.currentTarget.style.boxShadow = "0 8px 30px rgba(250,70,22,0.5)";
              }}
              onMouseLeave={(e) => {
                e.currentTarget.style.transform = "none";
                e.currentTarget.style.boxShadow = "0 4px 20px rgba(250,70,22,0.4)";
              }}
            >
            ...
            {isSignedIn ? (
              <>
                ...
                >
                  <Languages size={18} /> Practice Phrases
                </button>
                <button
              ...
              >
                <User size={18} /> Sign in to get started
              </button>
            )}
          </div>
        </div>
        ...
        <div
          style={{
            ...
            height: 500,
            borderRadius: "50%",
            background: "radial-gradient(circle,rgba(0,33,165,0.2),transparent 70%)",
            pointerEvents: "none",
          }}
        />
      </section>
          ...
          <div style={S.picksScroll}>
            {buildQuickPicks(userCities, userCityData?.slug).map((p, i) => (
              ...
              >
                ...
                <div>
                  <div style={{ fontSize: "0.68rem", fontWeight: 700, color: "#FA4616", textTransform: "uppercase", letterSpacing: "0.06em" }}>
                    {p.label}
                  </div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "2rem", flexWrap: "wrap" }}>
              <div style={{ flex: 1, minWidth: 280 }}>
                <div style={{ display: "inline-flex", alignItems: "center", gap: "0.4rem", padding: "0.3rem 0.8rem", borderRadius: 9999, background: "#FFF7ED", border: "1px solid #FED7AA", marginBottom: "0.75rem" }}>
                  ...
                  <span style={{ fontSize: "0.7rem", fontWeight: 600, color: "#EA580C", letterSpacing: "0.04em", textTransform: "uppercase" }}>
                    For UF Students
                  </span>
                </div>
                <h3 style={{ fontSize: "1.3rem", fontWeight: 800, color: "#111827", margin: "0 0 0.5rem", lineHeight: 1.3, letterSpacing: "-0.02em" }}>
                  ...
                  <span style={{ background: "linear-gradient(90deg,#FA4616,#FF6B35)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
                    personalized
                  </span>
                </h3>
                ...
                <div style={{ display: "flex", gap: "0.6rem", flexWrap: "wrap" }}>
                  <button
                    ...
                    style={{
                      ...
                      fontSize: "0.88rem",
                      fontWeight: 700,
                      boxShadow: "0 4px 16px rgba(0,33,165,0.25)",
                      transition: "all 0.2s",
                    }}
                    onMouseEnter={(e) => {
                      e.currentTarget.style.transform = "translateY(-1px)";
                      e.currentTarget.style.boxShadow = "0 8px 24px rgba(0,33,165,0.35)";
                    }}
                    onMouseLeave={(e) => {
                      e.currentTarget.style.transform = "none";
                      e.currentTarget.style.boxShadow = "0 4px 16px rgba(0,33,165,0.25)";
                    }}
                  >
      ...
      <section id="mtb-map" style={S.mapSection}>
        <div style={S.mapToolbar}>
          <div style={S.mapSearchWrap}>
            ...
            <input
              ...
              placeholder={userCities.length > 0 ? 'Search or type "my"' : "Search cities..."}
              style={S.mapSearchInput}
            />
            {mapSearch && (
              <div style={S.mapSearchDrop}>
                {userCities.length > 0 && mapSearch.toLowerCase() === "my" && (
                  <div style={{ padding: "0.35rem 0.75rem", fontSize: "0.65rem", fontWeight: 700, color: "#6B7280", textTransform: "uppercase", letterSpacing: "0.08em", borderBottom: "1px solid #F3F4F6" }}>
                    📌 Your Cities
                  </div>
                )}
                {userCities.length > 0 &&
                  mapSearch.toLowerCase().startsWith("my") &&
                  userCities.map((slug) => {
                    ...
                    return (
                      <div
                        ...
                        style={{ ...S.mapSearchItem, backgroundColor: "#F0FDF4" }}
                      >
                        {PIN_EMOJI[c.country]} {c.name}{" "}
                        <span style={{ color: "#059669", fontSize: "0.7rem", fontWeight: 600 }}>📌 Saved</span>
                      </div>
                    );
                  })}
                {CITY_COORDS.filter(
                  (c) => c.name.toLowerCase().includes(mapSearch.toLowerCase()) && !mapSearch.toLowerCase().startsWith("my")
                )
                  .slice(0, 5)
                  .map((c) => (
                    <div
            ...
            {showRouteFinder && (
              ...
              >
                <div style={{ fontSize: "0.7rem", fontWeight: 700, color: "#374151", marginBottom: "0.5rem" }}>
                  Find a Direct Train Connection
                </div>

                <select
                  value={routeFrom}
                  onChange={(e) => setRouteFrom(e.target.value)}
                  style={{ width: "100%", padding: "0.4rem 0.6rem", borderRadius: "0.5rem", border: "1px solid #E5E7EB", fontSize: "0.75rem", marginBottom: "0.5rem", backgroundColor: "#fff" }}
                >
                  <option value="">From: Select city</option>
                  {CITY_COORDS.map((city) => (
                    <option key={city.slug} value={city.slug}>
                      {PIN_EMOJI[city.country]} {city.name}
                    </option>
                  ))}
                </select>
                ...
                <button
                  onClick={findRoute}
                  style={{ width: "100%", padding: "0.4rem", borderRadius: "0.5rem", border: "none", background: "linear-gradient(135deg,#0021A5,#003087)", color: "#fff", cursor: "pointer", fontSize: "0.75rem", fontWeight: 600, marginBottom: "0.5rem" }}
                >
                  Find Route
                </button>

                {foundRoute && (
                  <div style={{ backgroundColor: "#ECFDF5", border: "1px solid #A7F3D0", borderRadius: "0.5rem", padding: "0.5rem", marginTop: "0.5rem" }}>
                ...
                {foundRoute === null && routeFrom && routeTo && (
                  <div style={{ backgroundColor: "#FEF2F2", border: "1px solid #FECACA", borderRadius: "0.5rem", padding: "0.5rem", marginTop: "0.5rem" }}>
          ...
          <div style={S.flyBtns}>
            {[
              ...
              <button key={c.code} onClick={() => flyToCountry(c.code)} style={S.flyBtn}>
                {c.flag}
              </button>
            ))}
            <button
              onClick={() => {
                ...
                if (m) m.flyTo([48.5, 10.5], 6, { duration: 1 });
              }}
              style={S.flyBtn}
            >
              <Globe size={12} />
          ...
          <button
            ...
            style={{ ...S.routeToggle, ...(showRoutes ? { backgroundColor: "rgba(250,70,22,0.08)", color: "#FA4616", borderColor: "rgba(250,70,22,0.35)" } : {}) }}
          >
        ...
        <div style={S.mapLegend}>
          <span style={{ fontSize: "0.58rem", fontWeight: 700, color: "#9CA3AF", textTransform: "uppercase", letterSpacing: "0.06em", marginRight: "0.15rem" }}>
            Map Key
          </span>
          {Object.entries(PIN_COLORS).map(([k, c]) => (
            <span key={k} style={S.legI}>
              <span style={{ ...S.legD, backgroundColor: c }} /> {COUNTRY_FLAGS[k]} {COUNTRY_LABELS[k]}
            </span>
          ))}
          <span style={S.legI}>
            <span style={{ ...S.legD, backgroundColor: "#fff", border: "2px solid #D1D5DB" }} /> Landmarks <span style={{ fontSize: "0.55rem", color: "#9CA3AF" }}>(zoom in)</span>
          </span>
          {showRoutes && (
            <>
              <span style={{ width: 1, height: 14, background: "#E5E7EB", flexShrink: 0 }} />
              <span style={S.legI}>
                <span style={{ width: 14, height: 2, backgroundColor: "#FA4616", display: "inline-block", borderRadius: 1 }} /> Paid
              </span>
              <span style={{ ...S.legI, display: "flex", alignItems: "center", gap: "0.25rem" }}>
                <span style={{ width: 14, height: 2, backgroundColor: "#10B981", display: "inline-block", borderRadius: 1 }} /> Student Pass
                <a
                  href="https://int.bahn.de/en/faq/deutschlandticket-which-trains"
                  target="_blank"
                  rel="noopener noreferrer"
                  title="Learn which trains your student ticket covers"
                  style={{ display: "inline-flex", cursor: "pointer", color: "#10B981", fontSize: "0.5rem" }}
                >
                  <Info size={10} />
                </a>
              </span>
        ...
        {selectedPin && (
          <div style={S.cityPanel}>
            ...
            {selectedPin.uni && (
              <div style={S.cpRow}>
                <GraduationCap size={13} color="#0021A5" />
                <span style={{ fontSize: "0.78rem", fontWeight: 600, color: "#0021A5" }}>{selectedPin.uni}</span>
              </div>
            )}
            <p style={{ fontSize: "0.82rem", color: "#4B5563", lineHeight: 1.5, margin: "0 0 0.75rem 0" }}>{selectedPin.desc}</p>
            {selectedPin.vibe && (
              <div style={S.cpVibe}>
                <Coffee size={12} /> Vibe: <strong>{selectedPin.vibe}</strong>
              </div>
            )}
            <div style={S.cpStats}>
              {selectedPin.cost && (
                <div style={S.cpStat}>
                  ...
                  <div>
                    <span style={{ fontSize: "0.58rem", fontWeight: 600, color: "#9CA3AF", display: "block", lineHeight: 1 }}>Avg Rent</span>
                    <span>{selectedPin.cost}</span>
                  </div>
                </div>
              )}
              {selectedPin.beer && (
                <div style={S.cpStat}>
                  ...
                  <div>
                    <span style={{ fontSize: "0.58rem", fontWeight: 600, color: "#9CA3AF", display: "block", lineHeight: 1 }}>Beer Price</span>
                    <span>{selectedPin.beer}</span>
                  </div>
                </div>
              )}
              {selectedPin.transit && (
                <div style={S.cpStat}>
                  <Train size={12} color="#4338CA" />
                  <div>
                    <span style={{ fontSize: "0.58rem", fontWeight: 600, color: "#9CA3AF", display: "block", lineHeight: 1 }}>Transit</span>
                    <span>{selectedPin.transit}</span>
                  </div>
                </div>
              )}
            </div>
            {selectedPin.mustDo && (
              <div style={S.cpMustDo}>
                <Star size={12} color="#059669" /> <span>{selectedPin.mustDo}</span>
              </div>
            )}
            ...
            {(() => {
              ...
              return (
                <div style={{ marginTop: "0.75rem" }}>
                  <div style={{ fontSize: "0.72rem", fontWeight: 700, color: "#374151", marginBottom: "0.4rem", display: "flex", alignItems: "center", gap: "0.3rem" }}>
                    <MapPin size={12} /> {nearbyLandmarks.length} places to visit
                  </div>
                  {nearbyLandmarks.map((lm, i) => (
                    <div
                      key={i}
                      style={{ ...S.cpRoute, cursor: "pointer" }}
                      onClick={() => lm.url && window.open(lm.url, "_blank", "noopener,noreferrer")}
                    >
            ...
            {(() => {
              ...
              return (
                <div style={{ marginTop: "0.75rem" }}>
                  <div style={{ fontSize: "0.72rem", fontWeight: 700, color: "#374151", marginBottom: "0.4rem", display: "flex", alignItems: "center", gap: "0.3rem" }}>
                    <Train size={12} /> {routes.length} Weekend Trip{routes.length !== 1 ? "s" : ""}
                  </div>
                  {freeRoutes.length > 0 && (
                    <div style={{ background: "#ECFDF5", border: "1px solid #A7F3D0", borderRadius: "0.4rem", padding: "0.35rem 0.55rem", marginBottom: "0.35rem", fontSize: "0.68rem", color: "#065F46", fontWeight: 600, display: "flex", alignItems: "center", justifyContent: "space-between", gap: "0.4rem" }}>
                      ...
                      <a href="https://int.bahn.de/en/faq/deutschlandticket-which-trains" target="_blank" rel="noopener noreferrer" style={{ flexShrink: 0, fontSize: "0.6rem", color: "#059669", textDecoration: "underline", fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap" }}>
                        Info
                      </a>
                    </div>
                  )}
            ...
            <div style={{ display: "flex", gap: "0.5rem", marginTop: "1rem" }}>
              <button
                onClick={() => navigate("/tips?city=" + selectedPin.slug)}
                style={S.cpPrimary}
                onMouseEnter={(e) => {
                  e.currentTarget.style.transform = "translateY(-1px)";
                  e.currentTarget.style.boxShadow = "0 6px 18px rgba(0,33,165,0.35)";
                }}
                onMouseLeave={(e) => {
                  e.currentTarget.style.transform = "none";
                  e.currentTarget.style.boxShadow = "0 3px 10px rgba(0,33,165,0.25)";
                }}
              >
                <Compass size={14} /> Explore
              </button>
              <button
                onClick={() => navigate("/events?city=" + encodeURIComponent(selectedPin.name))}
                style={S.cpSecondary}
                onMouseEnter={(e) => {
      ...
      {userCities.length > 0 ? (
        <section style={{ maxWidth: 1200, margin: "0 auto", padding: "2.5rem 2rem 1.5rem" }}>
          <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", flexWrap: "wrap", gap: "0.5rem", marginBottom: "1.75rem" }}>
            <div>
              <p style={{ fontSize: "0.68rem", fontWeight: 700, color: "#FA4616", textTransform: "uppercase", letterSpacing: "0.1em", margin: "0 0 0.25rem" }}>
              ...
              <p style={{ fontSize: "0.82rem", color: "#9CA3AF", margin: "0.25rem 0 0" }}>
                Your {userCities.length === 1 ? "city" : `${userCities.length} cities`}. Today's snapshot.
              </p>
            </div>
            <button
              onClick={() => {
              ...
              style={{ display: "inline-flex", alignItems: "center", gap: "0.35rem", padding: "0.5rem 1.1rem", border: "1.5px solid #E5E7EB", borderRadius: "0.6rem", background: "#fff", cursor: "pointer", fontSize: "0.78rem", fontWeight: 600, color: "#374151", transition: "all 0.15s" }}
              onMouseEnter={(e) => {
                e.currentTarget.style.borderColor = "#0021A5";
                e.currentTarget.style.color = "#0021A5";
              }}
              onMouseLeave={(e) => {
                e.currentTarget.style.borderColor = "#E5E7EB";
          ...
          {(() => {
            ...
            const TONIGHT_BY_CITY = {
              ...
              hamburg: { sunday: "🐟 Fischmarkt ends at 9:30am — go straight from last night or rise early", monday: "🎵 Elbphilharmonie has Monday concerts from €15 — worth every cent", tuesday: "🍸 Strandperle beach bar is the Tuesday local ritual", wednesday: "🎤 Molotow hosts mid-week indie gigs — student door deals before 10pm", thursday: "🎭 Reeperbahn Festival fringe events happen all year — check locally", friday: "🛥️ Alster boat bars start up — sunset cruise drinks from the dock", saturday: "🌟 Schanzenviertel wakes up at midnight — bars till 5am" },
              vienna: { sunday: "🎼 Vienna Boys Choir sings Sunday morning mass — €12 standing room", monday: "☕ Kaffeehäuser are perfect on slow Mondays — read the newspaper like a local", tuesday: "🍷 Heuriger wine taverns open from 4pm — CHF3 Grüner Veltliner", wednesday: "🎵 Volksoper has weekday discount tickets from €8 for students", thursday: "🪩 Gürtel bar street warms up — students spill out of the arches at 11pm", friday: "🍽️ Naschmarkt shuts and transforms into a late-night bar district", saturday: "🌟 Donaukanal beach bars are packed — Flex and Motto open till 6am" },
            ...
            const getDailyBookmarks = () => {
              try {
                return JSON.parse(localStorage.getItem("dailyPhraseBookmarks") || "[]");
              } catch {
                return [];
              }
            };
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(320px,1fr))", gap: "1.25rem" }}>
                {loading ? (
                  ...
                  sortedCities.map((slug, cardIdx) => {
                    ...
                    const upcomingEvents = citySavedEvents
                      .filter((event) => {
                    ...
                    const weather = weatherMap[slug] ?? {
                      icon: "⏳", cond: "Loading...", hi: "—", lo: "—", sunrise: "—", sunset: "—",
                    };
                    ...
                    const hdrGradient = isPrimary ? "linear-gradient(135deg,#FA4616 0%,#c73800 100%)" : "linear-gradient(135deg,#0021A5 0%,#003087 100%)";
                    ...
                    return (
                      <div
                        ...
                        style={{
                          ...
                          overflow: "hidden",
                          boxShadow: isPrimary ? "0 6px 28px rgba(250,70,22,0.12)" : "0 4px 20px rgba(0,0,0,0.07)",
                          border: isPrimary ? "1.5px solid #FDBA74" : "1px solid #E5E7EB",
                          background: "#fff",
                          transition: "all 0.3s cubic-bezier(0.16,1,0.3,1)",
                          gridColumn: isPrimary && userCities.length > 1 ? "1 / -1" : "auto",
                        }}
                        onMouseEnter={(e) => {
                          e.currentTarget.style.transform = "translateY(-4px)";
                          e.currentTarget.style.boxShadow = isPrimary ? "0 16px 48px rgba(250,70,22,0.15)" : "0 16px 48px rgba(0,33,165,0.13)";
                        }}
                        onMouseLeave={(e) => {
                          e.currentTarget.style.transform = "none";
                          e.currentTarget.style.boxShadow = isPrimary ? "0 6px 28px rgba(250,70,22,0.12)" : "0 4px 20px rgba(0,0,0,0.07)";
                        }}
                      >
                        <div style={{ background: hdrGradient, padding: isPrimary ? "1.1rem 1.35rem" : "0.85rem 1.15rem", position: "relative", overflow: "hidden" }}>
                          ...
                          <div style={{ position: "relative", zIndex: 1, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                            <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                              {isPrimary && (
                                <span style={{ fontSize: "0.5rem", fontWeight: 800, background: "rgba(255,255,255,0.25)", color: "#fff", padding: "0.12rem 0.5rem", borderRadius: 9999, letterSpacing: "0.08em", border: "1px solid rgba(255,255,255,0.35)" }}>
                                  ❖ YOUR CITY
                                </span>
                              )}
                        ...
                        {isPrimary ? (
                          <div style={{ padding: "1rem 1.25rem" }}>
                            <div style={{ display: "grid", gridTemplateColumns: userCities.length > 1 ? "1fr 1fr" : "1fr", gap: "0.75rem" }}>
                              <div
                                style={{ background: "linear-gradient(135deg,#EFF6FF,#DBEAFE)", borderRadius: "0.85rem", padding: "0.85rem 1rem", border: "1px solid #BFDBFE", transition: "all 0.2s" }}
                              ...
                              >
                                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "0.4rem" }}>
                                  ...
                                  <div style={{ display: "flex", alignItems: "center", gap: "0.3rem" }}>
                                    <button
                                      onClick={(e) => { e.stopPropagation(); toggleDailyBookmark(phrase); }}
                                      title={isDailyBookmarked(phrase.de) ? "Remove bookmark" : "Bookmark"}
                                      style={{ background: isDailyBookmarked(phrase.de) ? "#FFF7ED" : "#fff", border: isDailyBookmarked(phrase.de) ? "1px solid #FA4616" : "1px solid #BFDBFE", borderRadius: "0.35rem", width: 24, height: 24, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", transition: "all 0.15s", padding: 0 }}
                                    >
                                    ...
                                    <button
                                      ...
                                      style={{ background: "#fff", border: "1px solid #BFDBFE", borderRadius: "0.35rem", width: 24, height: 24, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", transition: "all 0.15s", padding: 0 }}
                                    >
                              ...
                              <div style={{ display: "flex", alignItems: "flex-start", gap: "0.6rem", background: "linear-gradient(135deg,#FFF8ED,#FFFBE8)", borderRadius: "0.85rem", padding: "0.75rem 0.9rem", border: "1px solid #FDEAB8" }}>
                                {(() => {
                                  return (
                                    <div style={{ display: "flex", alignItems: "flex-start", gap: "0.6rem", background: "linear-gradient(135deg,#FFF8ED,#FFFBE8)", borderRadius: "0.85rem", padding: "0.75rem 0.9rem", border: "1px solid #FDEAB8", width: "100%" }}>
                              ...
                              <div style={{ display: "flex", alignItems: "flex-start", gap: "0.6rem", background: "linear-gradient(135deg,#F0FFF4,#ECFDF5)", borderRadius: "0.85rem", padding: "0.75rem 0.9rem", border: "1px solid #A7F3D0" }}>
                                {(() => {
                                  ...
                                  return (
                                    <div style={{ display: "flex", alignItems: "flex-start", gap: "0.6rem", background: "linear-gradient(135deg,#F0FFF4,#ECFDF5)", borderRadius: "0.85rem", padding: "0.75rem 0.9rem", border: "1px solid #A7F3D0", width: "100%" }}>
                                      ...
                                      <div style={{ flex: 1, minWidth: 0 }}>
                                        ...
                                        <p style={{ fontSize: "0.75rem", color: "#064E3B", margin: 0, lineHeight: 1.4, fontWeight: 500 }}>
                                          🎉 {next?.name} in <strong>{daysUntil}</strong> day{daysUntil !== 1 ? "s" : ""}
                                        </p>
                                      </div>
                                    </div>
                                  );
                          ...
                          <div style={{ padding: "0.85rem 1.1rem", display: "flex", flexDirection: "column", gap: "0.6rem" }}>
                            ...
                            <div style={{ display: "flex", alignItems: "flex-start", gap: "0.55rem" }}>
                              ...
                              <div style={{ flex: 1, minWidth: 0 }}>
                                ...
                                <p style={{ fontSize: "0.72rem", color: "#064E3B", margin: 0, lineHeight: 1.35, fontWeight: 500 }}>
                                  {["Mensa lunch averages €3.50 — cheapest hot meal in town", "Bakery Brötchen before 8am are usually €0.40–€0.80", "Lidl/Aldi meal deals beat any takeout — check the weekly flyer", "Döner kebab is the student staple — €4–6 for a filling meal", "Cooking at home? Aldi has full meals under €2 per portion"][(dayIdx + pin.name.length) % 5]}
                                </p>
                              </div>
                            </div>
                          </div>
        ...
        <section style={{ maxWidth: 1200, margin: "0 auto", padding: "2.5rem 2rem 1.5rem" }}>
          <div style={{ background: "#fff", borderRadius: "1rem", border: "1px solid #E5E7EB", padding: "2.5rem", textAlign: "center" }}>
            <div style={{ width: 56, height: 56, borderRadius: "50%", background: "#EFF6FF", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 1rem" }}>
              <MapPin size={24} color="#0021A5" />
            </div>
            <h3 style={{ fontSize: "1.2rem", fontWeight: 700, color: "#111827", margin: "0 0 0.4rem" }}>Save cities to personalize your experience</h3>
            <p style={{ fontSize: "0.88rem", color: "#6B7280", maxWidth: 420, margin: "0 auto 1.25rem", lineHeight: 1.55 }}>
              Head to Explore, pick the cities you're studying in or want to visit, and they'll show up here as your personal hub.
            </p>
            <button
              onClick={() => navigate("/tips")}
              style={{ display: "inline-flex", alignItems: "center", gap: "0.4rem", padding: "0.7rem 1.5rem", border: "none", borderRadius: "0.6rem", background: "linear-gradient(135deg,#0021A5,#003087)", color: "#fff", cursor: "pointer", fontSize: "0.88rem", fontWeight: 700, boxShadow: "0 3px 12px rgba(0,33,165,0.2)" }}
            >
              <Compass size={15} /> Browse Cities
            </button>
          </div>
        </section>
      ) : null}
          ...
          <div style={{ background: "#fff", borderRadius: "1.25rem", width: "90%", maxWidth: 560, maxHeight: "80vh", overflow: "hidden", boxShadow: "0 25px 60px rgba(0,0,0,0.3)", display: "flex", flexDirection: "column" }}>
            <div style={{ padding: "1.75rem 2rem 1rem", borderBottom: "1px solid #F3F4F6" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.5rem" }}>
                <div style={{ width: 40, height: 40, borderRadius: "0.75rem", background: "linear-gradient(135deg,#0021A5,#003087)", display: "flex", alignItems: "center", justifyContent: "center" }}>
              ...
              {["DE", "AT", "CH"].map((code) => {
                ...
                return (
                  <div key={code} style={{ marginBottom: "1rem" }}>
                    <div style={{ fontSize: "0.72rem", fontWeight: 700, color: "#6B7280", marginBottom: "0.5rem", textTransform: "uppercase", letterSpacing: "0.06em" }}>
                      {PIN_EMOJI[code]} {COUNTRY_LABELS[code]}
                    </div>
                    <div style={{ display: "flex", gap: "0.4rem", flexWrap: "wrap" }}>
                      {countryCities.map((city) => {
                        ...
                        return (
                          <button
                            key={city.slug}
                            onClick={() => setPickerSelected((prev) => (selected ? prev.filter((s) => s !== city.slug) : [...prev, city.slug]))}
                            style={{
            ...
            <div style={{ padding: "1rem 2rem 1.5rem", borderTop: "1px solid #F3F4F6", display: "flex", gap: "0.75rem", justifyContent: "flex-end", alignItems: "center" }}>
              ...
              <button
                ...
                style={{ padding: "0.55rem 1.2rem", border: "1px solid #E5E7EB", borderRadius: "0.6rem", background: "#fff", cursor: "pointer", fontSize: "0.82rem", fontWeight: 600, color: "#6B7280" }}
              >
                Skip
              </button>
              <button
                ...
                style={{
                  ...
                  fontSize: "0.82rem",
                  fontWeight: 700,
                  boxShadow: pickerSelected.length > 0 ? "0 3px 12px rgba(250,70,22,0.3)" : "none",
                }}
              >
                <Compass size={14} /> Let's Go!
              </button>
            </div>
          </div>
        </div>
      ...
      <footer style={S.foot}>
        ...
        <p style={{ margin: "0.35rem 0 0", fontSize: "0.68rem", color: "#D1D5DB" }}>
          {TOTAL_CITIES} cities · 3 countries · 1 semester to explore them all
        </p>
      </footer>
    </div>
  );
...
if (typeof document !== "undefined") {
  const el = document.getElementById("mtb-anim") || document.createElement("style");
  el.id = "mtb-anim";
  el.textContent = `
    @keyframes slideUp { from { opacity: 0; transform: translateY(20px) } to { opacity: 1; transform: translateY(0) } }
    @keyframes mtbPulse { 0% { transform: translateX(-50%) scale(1); opacity: 0.25 } 50% { transform: translateX(-50%) scale(2.2); opacity: 0 } 100% { transform: translateX(-50%) scale(1); opacity: 0.25 } }
    @keyframes dashFlow { to { stroke-dashoffset: -16 } }
    .mtb-pulse { animation: mtbPulse 2.5s ease-out infinite }
    .mtb-route { animation: dashFlow 1s linear infinite }
    .city-tooltip .leaflet-tooltip-content { font-family: Inter, system-ui, sans-serif }
    .leaflet-tooltip { border: 1px solid #E5E7EB !important; border-radius: 0.85rem !important; padding: 0 !important; box-shadow: 0 12px 36px rgba(0,0,0,0.18) !important; max-width: 340px !important; width: max-content !important; min-width: 200px !important; overflow: visible !important; white-space: normal !important; word-wrap: break-word !important; overflow-wrap: break-word !important; line-height: 1.4 !important }
    .leaflet-tooltip * { max-width: 100% !important; overflow-wrap: break-word !important; word-break: break-word !important }
  `;
